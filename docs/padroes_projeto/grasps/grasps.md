...
Eles classificam tanto problemas quanto soluções como padrões e, após as definições destes, podem então ser aplicados em instânias parecidas.
...
Foram utilizados os padrões XX, XX, XX e XX no projeto. Suas aplicações no projeto serão mais detalhados a baixo.
